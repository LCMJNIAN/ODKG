		...
		<el-row :gutter="20">
			<el-col :span="24">
				<el-table :data="tableData.slice((queryInfo.page - 1)*queryInfo.pageSize, queryInfo.page*queryInfo.pageSize)" size='small' style="width: 100%" :default-sort="{prop:'create_time', order:'descending'}">
					...
					<el-table-column prop="file_name" label="名称" width="120%" align="center">
					</el-table-column>
					<!-- <el-table-column prop="task_id" label="任务" width="150">
					</el-table-column> -->
					<el-table-column prop="file_path" label="文件路径" width="400%" align="center">
					</el-table-column>
					<el-table-column prop="create_time" label="创建时间" width="160%" :sortable="true" align="center">
					</el-table-column>
					<el-table-column prop="address" label="操作"  align="center">
						<template slot-scope="scope">
							<el-button size="small" :plain="true" @click="sendRequest(scope.row.task_id, scope.row.file_path)">智能归档</el-button>
							<el-button size="small" :plain="true" type="info"  @click="checkStatus(scope.row.task_id)">查看状态</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			// 检查归档状态
			checkStatus(task_id) {
				axios.get("/api/classification/status", {
					params: {
						task_id: task_id
					}
				}).then(result => {
			...
			sendRequest(task_id, file_path) {
				...
				axios.post("/api/classification", params).then(result => {
					this.$message('请求已发送');
				})
			},
			// 查看归档结果
			checkResult(task_id) {
				...
				params.batch_num = "1";
				axios.post("/api/classification/result", params).then(result => {
					console.log(result.data.message);
					this.resultData = result.data.data;
				})
			},
			//下载文件
			downloadFile(task_id = 300, task_type = "归档") {
				axios.get("/api/download", {
					...
					link.target = '_blank'
					let filename = `任务${task_id}结果归档.csv`
