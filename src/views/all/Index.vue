...
<script>
	...
	export default {
		...
		methods: {
			// 检查归档状态
			checkStatus(task_id) {
				axios.get("http://10.160.181.146:5000/all/status", {
					params: {
						task_id: task_id
					}
				}).then(result => {
			...
			sendRequest(task_id, file_path) {
				...
				axios.post("http://10.160.181.146:5000/all/extract", params).then(result => {
					this.$message('请求已发送');
				})
			},
			// 查看抽取结果
			checkResult(task_id) {
				...
				params.batch_num = "1";
				axios.post("http://10.160.181.146:5000/general/result", params).then(result => {
			...
			//下载文件
			downloadFile(task_id = 300, task_type = "一键三连") {
				axios.get("http://10.160.181.146:5000/download", {
					params: {
						task_id,
						task_type
					},
					...
					link.target = '_blank'
					let filename = `任务${task_id}结果归档抽取.csv`
			...
			deleteTaskId(task_id, item) {
				...
				params.task_id = task_id;
                params.task_type = "一键三连";
				axios.post("http://10.160.181.146:5000/delete", params).then(result => {
					console.log(result.data.message);
				})
				this.$message({
					message:"任务删除成功",
