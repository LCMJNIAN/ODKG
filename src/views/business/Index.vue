	...
	export default {
		...
		methods: {
			...
			sendRequest(task_id, file_path) {
				...
				axios.post("/api/business/extract", params).then(result => {
					this.$message('请求已发送');
				})
			},
			// 查看抽取结果
			checkResult(task_id) {
				...
				params.batch_num = "1";
				axios.post("/api/business/result", params).then(result => {
			...
			//下载文件
			downloadFile(task_id = 300, task_type = "业务抽取") {
